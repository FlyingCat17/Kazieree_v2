...
public class laporanBulan extends javax.swing.JFrame {
    ...
    public void getPendapatan() {
        try { //Pendapatan
            String sql = "SELECT SUM(jual.total_harga) AS Income FROM jual\n"
                    + "WHERE EXTRACT(MONTH FROM tgl_transaksi) = '" + bulan + "'\n"
                    + "AND EXTRACT(YEAR FROM tgl_transaksi) = '" + tahun + "'\n"
                    + "GROUP BY EXTRACT(MONTH FROM jual.tgl_transaksi)";
    ...
    public void getLabaBersih() {
        ...
        try {
            String tgll = tahun + "-" + bulan;
            String sqlBeban = "SELECT SUM(pengeluaran.jumlah_pengeluaran) AS Beban FROM pengeluaran\n"
                    + "WHERE EXTRACT(YEAR FROM pengeluaran.tgl_pengeluaran) = '" + tahun + "'\n"
                    + "AND EXTRACT(MONTH FROM pengeluaran.tgl_pengeluaran) = '" + bulan + "'";
            java.sql.Connection con = (Connection) configdb.GetConnection();
            java.sql.Statement st = con.createStatement();
...
//        String tgll = tahun + "-" + bulan + "-" + hari;
        try {
            String loadPengeluaran = "SELECT SUM(pengeluaran.jumlah_pengeluaran) AS Outcome FROM pengeluaran\n"
                    + "WHERE EXTRACT(YEAR FROM tgl_pengeluaran) = '" + tahun + "'\n"
                    + "AND EXTRACT(MONTH FROM tgl_pengeluaran) = '" + bulan + "'";
            System.out.println(loadPengeluaran);
            java.sql.Connection con = (Connection) configdb.GetConnection();
            java.sql.Statement st = con.createStatement();
    ...
    public void getDataTransaksiJual() {
        ...
        transjual.addColumn("Pendapatan");
        table_transaksijual.setModel(transjual);
        try {
            String sql = "SELECT cast(tgl_transaksi as date) AS tanggal, SUM(jual.total_harga) AS Pendapatan\n"
                    + "FROM jual\n"
                    + "WHERE EXTRACT(MONTH FROM jual.tgl_transaksi) = '" + bulan + "'\n"
    ...
    public void getDataTransaksiBeli() {
        ...
        transbeli.addColumn("Tanggal");
        transbeli.addColumn("Total Pembelian");
        table_transaksibeli.setModel(transbeli);
        try {
            String sql = "SELECT CAST(beli.tgl_transaksi AS DATE) AS tanggal, SUM(beli.total_harga) AS total_beli\n"
                    + "FROM beli\n"
                    + "WHERE EXTRACT(MONTH FROM beli.tgl_transaksi) = '"+bulan+"'\n"
                    + "AND EXTRACT(YEAR FROM beli.tgl_transaksi) = '"+tahun+"'\n"
                    + "GROUP BY CAST(beli.tgl_transaksi AS DATE)";
            java.sql.Connection con = (Connection) configdb.GetConnection();
            java.sql.Statement st = con.createStatement();
            java.sql.ResultSet rs = st.executeQuery(sql);
            while (rs.next()) {
    ...
    public void getDataRataRataProdukBarang() {
        DefaultTableModel ratarata = new DefaultTableModel();
        ratarata.addColumn("Kode Produk");
        ratarata.addColumn("Nama Produk");
        ratarata.addColumn("Rata Rata");
        table_ratarata.setModel(ratarata);
        try {
            String sql = "SELECT detail_jual.kode_produk, detail_jual.nama_produk, ROUND(SUM(detail_jual.qty / 4)) AS Rata_Rata\n"
                    + "FROM detail_jual\n"
                    + "JOIN jual ON detail_jual.kode_transaksi = jual.kode_transaksi\n"
                    + "WHERE EXTRACT(MONTH FROM jual.tgl_transaksi) = '" + bulan + "'\n"
                    + "AND EXTRACT(YEAR FROM jual.tgl_transaksi) = '" + tahun + "'\n"
                    + "GROUP BY detail_jual.kode_produk\n"
                    + "ORDER BY ROUND(SUM(detail_jual.qty / 4)) DESC";
            java.sql.Connection con = (Connection) configdb.GetConnection();
            java.sql.Statement st = con.createStatement();
            java.sql.ResultSet rs = st.executeQuery(sql);
            while (rs.next()) {
                Double db = Double.parseDouble(rs.getString(3));
                int i = (int) Math.round(db);
                if (i != 0) {
                    System.out.println(i);
                    ratarata.addRow(new Object[]{
                        rs.getString(1),
                        rs.getString(2),
                        i
                    });
            ...
            System.err.println("Error Load ratarata : " + e.getMessage());
        }
    }
    ...
    private void initComponents() {
        ...
        table_ratarata = new com.fathan.swing.jtable.TableDark(){
        ...
        table_ratarata.setForeground(new java.awt.Color(51, 51, 51));
        table_ratarata.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null},
        ...
        table_ratarata.setFont(new java.awt.Font("Montserrat Medium", 0, 12)); // NOI18N
        table_ratarata.setGridColor(new java.awt.Color(255, 255, 255));
        table_ratarata.getTableHeader().setResizingAllowed(false);
        table_ratarata.getTableHeader().setReorderingAllowed(false);
        table_ratarata.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                table_ratarataMouseClicked(evt);
            }
        });
        jScrollPane4.setViewportView(table_ratarata);
        ...
        jLabel10.setText("Rata-Rata Produk Barang yang Terjual");
        panelShadow9.add(jLabel10, new org.netbeans.lib.awtextra.AbsoluteConstraints(50, 40, -1, 30));

        jPanel1.add(panelShadow9, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 900, 1090, 410));
        ...
        jPanel1.add(panelShadow10, new org.netbeans.lib.awtextra.AbsoluteConstraints(590, 290, 360, 160));
        ...
        panelShadow8.add(jLabel9, new org.netbeans.lib.awtextra.AbsoluteConstraints(30, 30, 280, -1));
        ...
        jPanel1.add(panelShadow8, new org.netbeans.lib.awtextra.AbsoluteConstraints(220, 290, 360, 160));
        ...
        jLabel3.setText("Laporan Harian >");
        ...
        panelShadow6.add(jScrollPane3, new org.netbeans.lib.awtextra.AbsoluteConstraints(50, 90, 440, 280));

        jLabel8.setFont(new java.awt.Font("Montserrat ExtraBold", 0, 20)); // NOI18N
        jLabel8.setForeground(new java.awt.Color(19, 179, 200));
        jLabel8.setText("Pembelian per Hari");
        panelShadow6.add(jLabel8, new org.netbeans.lib.awtextra.AbsoluteConstraints(50, 40, -1, 30));

        jPanel1.add(panelShadow6, new org.netbeans.lib.awtextra.AbsoluteConstraints(590, 470, 540, 410));
        ...
        panelShadow5.add(jScrollPane2, new org.netbeans.lib.awtextra.AbsoluteConstraints(50, 90, 420, 280));

        jLabel6.setFont(new java.awt.Font("Montserrat ExtraBold", 0, 20)); // NOI18N
        jLabel6.setForeground(new java.awt.Color(19, 179, 200));
        jLabel6.setText("Penjualan per Hari");
        panelShadow5.add(jLabel6, new org.netbeans.lib.awtextra.AbsoluteConstraints(50, 40, -1, 30));

        jPanel1.add(panelShadow5, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 470, 540, 410));
        ...
        label_ucapan1.setText("Laporan Bulanan");
    ...
    private void table_ratarataMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_table_ratarataMouseClicked
        // TODO add your handling code here:
    }//GEN-LAST:event_table_ratarataMouseClicked
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(laporanBulan.class
    ...
    private com.fathan.swing.jtable.TableDark table_ratarata;
